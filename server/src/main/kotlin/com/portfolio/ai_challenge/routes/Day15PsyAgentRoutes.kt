...
fun Route.day15PsyAgentRoutes(
    ...
    route("/api/agent/psy15") {
        ...
        get("/phase") {
            ...
            try {
                val (phase, allowed) = agent.getPhaseInfo(sessionId)
                call.respond(PhaseInfoResponse(taskPhase = phase, allowedTransitions = allowed))
            } catch (e: Exception) {
                call.respond(HttpStatusCode.NotFound, mapOf("error" to (e.message ?: "Session not found")))
            }
        }
